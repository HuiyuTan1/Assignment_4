...
GrowthSW<- GrowthSW|>
  mutate(treat=if_else(revolutions > 0, "YES","NO"))
datasummary((growth+rgdp60)*treat~(Mean+Median+sd+Max+min),(data=GrowthSW))

```

**Designated place**: To the both statistics of `growth` and `rgdp60`, the means and medians of them when countries had no revolution happened are greater than the numbers when countries have at least one revolution.
...
```{r}
# write t test here
treatment_group <- GrowthSW$growth[GrowthSW$treat=="YES"]
control_group <- GrowthSW$growth[GrowthSW$treat=="NO"]
...
# The p-value represents probability 
...
#The variable 'rgdp60' means value of GDP per capita in 1960, converted to 1960 US dollars.
